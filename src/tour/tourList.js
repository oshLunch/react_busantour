...
const TourList = () => {
  ...
  useEffect(() => {
    ...
    const tourData = async () => {
      /* try, catch */
      /* 에러가 발생할 때, 스크립트 중단 및 콘솔에 출력 */
      /* 에러가 없다면 try 내부 코드 실행 */
      /* 에러가 있다면 catch값으로 넘어감 */
      try {
        const response = await axios.get(
          `https://apis.data.go.kr/6260000/AttractionService/getAttractionKr?serviceKey=${key}&numOfRows=20&pageNo=1&resultType=json`);
